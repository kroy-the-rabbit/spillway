...
// configMapReconcileConfig returns the type-specific reconcile configuration for
// the ConfigMapReconciler, used by the shared reconcileObject function.
func configMapReconcileConfig() reconcileConfig[*corev1.ConfigMap] {
	return reconcileConfig[*corev1.ConfigMap]{
		kind:                  "ConfigMap",
		replicaSourceFieldIdx: configMapReplicaSourceFieldIdx,
		applyData: func(src, target *corev1.ConfigMap, kf keyFilter) {
		...
		listReplicas: func(ctx context.Context, c client.Client, fieldIdx, sourceFrom string) ([]*corev1.ConfigMap, error) {
			var all corev1.ConfigMapList
			if err := c.List(ctx, &all, client.MatchingFields{fieldIdx: sourceFrom}); err != nil {
				return nil, err
			}
			out := make([]*corev1.ConfigMap, len(all.Items))
...
func (r *ConfigMapReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	...
	result, err := reconcileObject(ctx, r.Client, log, r.Recorder, r.SelfHealInterval, configMapReconcileConfig(), &src)
	if err != nil {
		return result, err
	}
	...
	if r.OrphanAuditInterval > 0 {
		...
		r.auditMu.Unlock()
		if due {
			if auditErr := r.auditOrphanedConfigMapReplicas(ctx); auditErr != nil {
...
// auditOrphanedConfigMapReplicas scans all managed ConfigMap replicas and deletes
// any whose source ConfigMap no longer exists. Profile replicas are skipped.
func (r *ConfigMapReconciler) auditOrphanedConfigMapReplicas(ctx context.Context) error {
	var all corev1.ConfigMapList
	...
	for i := range all.Items {
		...
		ns, name, ok := parseSourceFrom("ConfigMap", sourceFrom)
		if !ok {
			continue
		}
		var src corev1.ConfigMap
		if err := r.Get(ctx, types.NamespacedName{Namespace: ns, Name: name}, &src); err != nil {
			if apierrors.IsNotFound(err) {
				r.Log.Info("deleting orphaned ConfigMap replica: source no longer exists",
					"replica", client.ObjectKeyFromObject(replica).String(),
					"source", sourceFrom)
				if delErr := safeDelete(ctx, r.Client, replica); delErr != nil {
					r.Log.Error(delErr, "failed to delete orphaned ConfigMap replica")
				}
			}
		}
	}
...
func (r *ConfigMapReconciler) sourceRequestsForNamespace(ctx context.Context, obj client.Object) []ctrl.Request {
	...
	var all corev1.ConfigMapList
	if err := r.List(ctx, &all, client.MatchingFields{cmSourceTargetingFieldIdx: "true"}); err != nil {
...
func (r *ConfigMapReconciler) SetupWithManager(mgr ctrl.Manager) error {
	...
	if err := registerConfigMapSourceTargetingIndex(context.Background(), mgr); err != nil {
		return err
	}
