...
- Kubernetes 1.35 through 1.37 (see below)
- Helm 3.10+

**Supported Kubernetes versions.** spillway is tested against the last three
Kubernetes minor releases, currently 1.35 through 1.37 (it is built on
`k8s.io/client-go` v0.37). Older versions may work but are unsupported.
...
From v1.0.0, the following are stable for the whole 1.x line. Changes to them are additive only; anything removed goes through at least one minor release of deprecation warnings first.

- **Annotations** under `spillway.kroy.io/` listed in the [annotation contract](#annotation-contract): names, value syntax, and semantics.
- **`SpillwayProfile` `spillway.kroy.io/v1`**: the schema and reconciliation semantics. `v1alpha1` is served and marked deprecated; it is removed no earlier than v1.2.0.
...
- **Metrics** listed under [Metrics](#metrics): names and label sets.
...
1. Upgrade the chart as usual (`helm upgrade ... --version <release>`) to any 1.x release. The chart installs the CRD with both `v1` (storage) and `v1alpha1` (served, deprecated). Existing `SpillwayProfile` objects keep working; the API server converts between the two versions without a webhook because the schemas are identical.
